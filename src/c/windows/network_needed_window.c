#include "network_needed_window.h"
...
static void window_load(Window *window) {
  ...
  s_icon_bitmap = gbitmap_create_with_resource(RESOURCE_ID_NETWORK_REQUIRED);
  ...
  const GEdgeInsets title_insets = {.top = 0};
  s_title_layer = text_layer_create(grect_inset(bounds, title_insets));
  text_layer_set_text(s_title_layer, DIALOG_NETWORK_WINDOW_APP_NAME);
  ...
  text_layer_set_text(s_body_layer, DIALOG_NETWORK_WINDOW_MESSAGE);
